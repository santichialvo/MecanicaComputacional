function Ejercicio2_FVM_C2()

%Caso 2
% 0 < x < L
% phi(0)=0 ; dphi_dxL(1)=1
Gamma = 1;
Q = 10;
L = 1;
    ...
    for j=i:i+1 %cada cara
        ...
        if (j == 1) %primer cara
            ...
            if (j == cant_celdas+1) %ultima cara
                ...
                %K(i,j-1) = K(i,j-1) + (-1 * Gamma * (1/h1)); %este aporte
                %solo iria si fuese dirichlet
            else   
                if (left)
...
%Cond Neumann
F(cant_celdas) = F(cant_celdas) - dPhi_dxL;
