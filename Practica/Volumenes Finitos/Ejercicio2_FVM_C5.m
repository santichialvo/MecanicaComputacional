function Ejercicio2_FVM_C5()

%Caso 5
% 0 < x < L
% Mixta en x=1, h=10, Qinf=2
% phi(0)=0 ;

Gamma = 1;

phi0 = 0;
h = 0.01;
x = 0:h:1;

cant_celdas = length(x)-1;

Q = 10;
hinf = 10;
Qinf = 2;
K = zeros(cant_celdas,cant_celdas);
F = zeros(cant_celdas,1);
left = true;

%syms phi_left phi_cent phi_right;

for i=1:cant_celdas %cada celda
    
    for j=i:i+1 %cada cara
        
        if (j == 1) %primer cara
            %Eq = Eq + (-1 * Gamma * (1/h1) * (phi_cent - phi0));
            h1 = h/2;
            K(i,j) = K(i,j) + (-1 * Gamma * (1/h1));
            left = false;
        else
            if (j == cant_celdas+1) %ultima cara
                %Eq = Eq + (Gamma * (1/h1) * (phiL - phi_cent));
                h1 = h/2;
                K(i,j-1) = K(i,j-1) + ( (hinf*(Gamma/h1)) / (hinf + (Gamma/h1)) );
            else   
                if (left)
                    %Eq = Eq + (-1 * Gamma * (1/h) * (phi_left - phi_cent));
                    K(i,j) = K(i,j) + (-Gamma * (1/h));
                    K(i,j-1) = K(i,j-1) + (Gamma * (1/h));
                    left = false;
                else
                    if (~left)
                        %Eq = Eq + (Gamma * (1/h) * (phi_right - phi_cent));
                        K(i,j) = K(i,j) + Gamma * (1/h);
                        K(i,j-1) = K(i,j-1) + (-Gamma * (1/h));
                        left = true;
                    end
                end
            end     
        end
        
    end
    
    F(i) = -Q*h;
    
end

%Cond dirichlet
F(1) = F(1) - ((2*Gamma)/h * phi0);
%Cond mixta
F(cant_celdas) = F(cant_celdas) + Qinf*( (hinf*(Gamma/(h/2))) / (hinf + (Gamma/(h/2))) );

a = K\F;
%a(2:length(a)+1)=a;
%a(1)=phi0;
%a(length(a)+1)=phiL;
for i=1:cant_celdas
    xmid(i)= x(i)+(x(i+1)-x(i))/2;
end

%xmid(2:length(xmid)+1)=xmid;
%xmid(1) = x(1);
%xmid(length(xmid)+1)=x(length(x));
e=exp(1);

%% No estoy seguro de esta...
%exact = 10.*sqrt(2).*sec(sqrt(2)).*sin(sqrt(2).*xmid);

%err = sum((a - exact'));

%figure;
plot(xmid,a);


end