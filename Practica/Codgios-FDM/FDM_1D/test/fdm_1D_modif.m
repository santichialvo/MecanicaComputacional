...
dt = 100; %1e-3;            % paso de tiempo
ntime = 1; %20;        % numero de pasos de tiempo
...
nfreqplot = 1; %10;     % frecuencia de ploteo
...
elseif 0
    fixa(nodes_lef,:) = [1,1];  % el nodo a izquierda tiene temperatura fija en 1
    ...
    fixa(nodes_rig,:) = [1,1];  % en este caso el nodo a derecha tiene temperatura libre con flujo impuesto y unitario (q*n=1)    
end

% initial state
if initia
    state = xnod.^2;         % inicialmente la temperatura de todos los nodos es 0 
...
elseif 0
    data.param.type_solver = 'explicit';
    scaling = 0;
else
    data.param.type_solver = 'analytic';
    scaling = 0;
end
data.param.relax = 1;
data.param.ndf = ndf;
...
for itime=1:ntime
    ...
    while check_conv
        ...
        check_conv = (norm(R)>tol_newton)&(iter<=maxit) & strcmp(data.param.type_solver,'implicit');
        iter = iter+1;
        %keyboard;pause
    end
